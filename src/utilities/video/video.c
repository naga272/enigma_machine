...
u8 is_set_colors_cli = 0;
u8 is_set_username = 0;
u8 setup_finish = 0;
u8 idx_word = 0;
...
void gestisci_char_to_write(uchar tmp_char_container)
{
    ...
    if (CHAR_END_PHRASE(tmp_char_container)) {
        if (!try_execute_comm(buffer_line_cmd) && setup_finish)
            start_encryption();
...
O3 static inline void gestisci_char_x_username(struct user_table* table, uchar c)
{
    // terminal_writechar(tmp_char_container, actual_color_terminal);
    if (CHAR_BACKSPACE(c)) {
        if (idx_word > 0) {
            table->user.username[idx_word] = '\0';
            idx_word--;
            gestisci_char_to_write(c);
        }
        return;
    }
    ...
    if (CHAR_END_PHRASE(c)) {
        ...
        idx_word = 0;
        terminal_writechar('\n', actual_color_terminal);
        print((uchar*) "password: ");
        return;
    }

    table->user.username[idx_word] = c;
    gestisci_char_to_write(c);
    idx_word++;
}


O3 static inline void gestisci_char_x_password(struct user_table* table, uchar c)
{
    // terminal_writechar(tmp_char_container, actual_color_terminal);
    if (CHAR_BACKSPACE(c)) {
        if (idx_word > 0) {
            table->user.password[idx_word] = '\0';
            idx_word--;
            gestisci_char_to_write(c);
        }
        return;
    ...
    if (CHAR_END_PHRASE(c)) {
        table->user.password[idx_word] = '\0';
        terminal_writechar('\n', actual_color_terminal);
        print((uchar*) table->user.password);
        setup_finish++;
        idx_word = 0;
        return;
    }

    table->user.password[idx_word] = c;
    gestisci_char_to_write('*');
    idx_word++;
}


O3 static inline void get_name_and_passw(struct user_table* table, uchar c)
{
    if (!c)
        return;

    if (!is_set_username) {
        gestisci_char_x_username(table, c);
        return;
    }

    gestisci_char_x_password(table, c);
}


O3 void try_the_setup(struct user_table* table, uchar c)
{
    /*
    ...
    * **/
    ...
    /* Fin tanto che non risolvo il problema della lettura
    *    del disco, finisco in automatico il setup
    *    quando l'utente preme i tasti per scegliere
    *    il colore del terminale
    */
    if (!c)
        return;

    /* NOT YET READY
    if (is_set_colors_cli) {
        get_name_and_passw(table, c);
        return;
    }
    */
    ...
    table->user.actual_color_terminal = actual_color_terminal;
    /*
