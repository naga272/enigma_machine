...
O3 void enqueue_interrupt(struct regs_t *r, i32 priority) {
    while (atomic_read(&queue_int.locks_queue_updating) != 0)
        ;

    atomic_inc(&queue_int.locks_queue_updating);

    queue_int.queue[queue_int.tail].regs.int_no = r->int_no;

    queue_int.queue[queue_int.tail].regs.ds = r->ds;
    queue_int.queue[queue_int.tail].regs.es = r->es;
    queue_int.queue[queue_int.tail].regs.fs = r->fs;
    queue_int.queue[queue_int.tail].regs.gs = r->gs;

    queue_int.queue[queue_int.tail].regs.edi = r->edi;
    queue_int.queue[queue_int.tail].regs.esi = r->esi;
    queue_int.queue[queue_int.tail].regs.ebp = r->ebp;
    queue_int.queue[queue_int.tail].regs.esp = r->esp;
    queue_int.queue[queue_int.tail].regs.ebx = r->ebx;
    queue_int.queue[queue_int.tail].regs.edx = r->edx;
    queue_int.queue[queue_int.tail].regs.ecx = r->ecx;
    queue_int.queue[queue_int.tail].regs.eip = r->eip;
    queue_int.queue[queue_int.tail].regs.cs = r->cs;
    queue_int.queue[queue_int.tail].regs.eflags = r->eflags;

    queue_int.queue[queue_int.tail].priority = priority;
    queue_int.queue[queue_int.tail].is_occupated = 1; // opzionale

    queue_int.tail = (queue_int.tail + 1) % SIZE_LIST_QUEUE;

    atomic_dec(&queue_int.locks_queue_updating);
}
...
O3 void int0h_handler(struct regs_t *r)
{
    ...
    * */
    ...
    enqueue_interrupt(r, PRIORITY_EXC_CPU);
...
O3 void int20h_handler(struct regs_t *r)
{
    ...
    */
    ...
    enqueue_interrupt(r, PRIORITY_PIC_MASTER);
...
{   
    enqueue_interrupt(r, PRIORITY_PIC_SLAVE);
...
{
    enqueue_interrupt(r, PRIORITY_SYSCALL);
    return do_int80h(r);
}
...
O3 void idt_init()
{
    ...
    memset(&queue_int, 0, sizeof(struct queue));
