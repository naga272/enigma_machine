...
MODULE_DESCRIPTION("gestione configurazione username, password e colori shell");
...
uchar* magic_num_sec_config = (uchar*) "EnigmaOs";


uchar buf_config[512];


O3 ainline void test_disk()
{
    /*
    * Test formattazione settore SECTOR_CONFIG
    *  ____________________________________________________________
    * |                  SETTORE SECTOR_CONFIG (512 bytes)         |
    * |------------------------------------------------------------|
    ...
    * * */
    ...
    // firma
    for (; magic_num_sec_config[offset] != '\0'; offset++)
        buf[offset] = magic_num_sec_config[offset];
    ...
    disk_write_sector(SECTOR_CONFIG, 1, (void*) buf);
    disk_read_sector(SECTOR_CONFIG, 1, buf);

    print(buf);
}


O3 ainline void write_on_sector_config()
{
    /*
    *                Test formattazione settore SECTOR_CONFIG
    *  ____________________________________________________________
...
O3 ainline void display_initial_msg()
{
    uchar buf[598];

    i32 fd = fopen("0:/config.con", "r");
...
void init_setup(struct book* b)
{
    ...
    write_on_sector_config();
    print((uchar*) "\n>>> ");
}


O3 ainline void cmp_credentials()
{
...
O3 ainline i8 check_username(uchar char_pressed)
{
    if (!char_pressed)
        return 0;
...
O3 ainline i8 check_password(uchar char_pressed)
{
    if (!char_pressed)
        return 0;
...
O3 void do_login()
{
    ...
    kprintf("\nWelcome %s!", username_insert);
}


O3 void do_config(uchar *buf_config)
{
    terminal_initialize((u8) buf_config[73]);

    u8 offset = 9;

    for (u8 idx = 0; idx < 32; idx++, offset++)
        username[idx] = buf_config[offset];

    offset++;

    for (u8 idx = 0; idx < 32; idx++, offset++)
        password[idx] = buf_config[offset];
